...
def backup_full_database():
    """Exporta todas as collections do database 'upik-chat-prod' em um único .json.gz"""
    mongo_cfg = Variable.get("mongodb")
    client = MongoClient(mongo_cfg["ConnectionString"])
    db = client["upik-chat-prod"]
...
@flow(name="backup-chat-prod-database")
def backup_chat_prod_database_flow():
